...
def preprocess_image(
    ...
    **rembg_kwargs,
    ...
    if isinstance(foreground_ratio, str):
        try:
            foreground_ratio = float(foreground_ratio)
        except ValueError:
            print(f"Invalid foreground_ratio: {foreground_ratio}, using default 0.9")
    ...
    for i in range(len(images_pil)):
        ...
        x1, y1, x2, y2 = bboxs
        dy, dx = y2 - y1, x2 - x1
        s = min(float(height) * float(foreground_ratio) / float(dy), 
        float(width) * float(foreground_ratio) / float(dx))
...
def import_mesh(mesh):
    ...
    if isinstance(mesh, str):
        ...
        mesh_pymeshlab = pymeshlab.Mesh(
            vertex_matrix=original_mesh.verts.cpu().numpy(), 
            face_matrix=original_mesh.faces.cpu().numpy()
        )
